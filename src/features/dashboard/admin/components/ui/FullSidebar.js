...
export default function FullSidebar({ toggelSidebar, closeSidebar }) {
   ...
   const { data: dashboardMenuItems } = useGetAdminDashboardMenuQuery('admin_dashboard')
   const handleDropdown = (name) => {
      if (dropdownOpen.includes(name)) {
         const index = dropdownOpen.indexOf(name);
         dropdownOpen.splice(index, 1);
      } else {
         dropdownOpen.push(name);
      }
   }
   ...
   //       </svg>,
   //       url: '/admin/dashboard',
   //       path: ['/admin', '/admin/', '/admin/dashboard'],
   //       onClickAction: '',
   //       dropdownMenu: ''
   //    },
   //    {
         ...
         <div className="h-full px-3 py-4 overflow-y-auto bg-gray-50 w-64 dark:bg-gray-800">
            ...
            <div className="w-full px-2 overflow-y-auto">
               <div className="flex flex-col items-center w-full mt-3 border-t border-gray-700">
                  ...
                  {dashboardMenuItems?.data.map((item, index) => {
                     if (item.dropdownMenu) {
                        return (<div key={index} className='w-full'>
                           <MenuLink action={handleDropdown} link={item} />
                           <Dropdown items={dropdownMenu} visible={dropdownOpen.includes(item.title)} />
                        </div>)
                     } else {
