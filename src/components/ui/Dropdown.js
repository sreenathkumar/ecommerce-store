...
export default function Dropdown({ allItems, visible }) {
            ...
            <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
               {items?.map((item, index) => {
                  if (item.onClick && !item.url) {
                     ...
                     return <li key={index} >
                        <Link to={item.url} className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{item.name}</Link>
                     </li>
                  }
               })}
            </ul>
